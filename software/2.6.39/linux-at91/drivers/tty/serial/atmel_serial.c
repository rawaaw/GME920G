...
#if defined CONFIG_ARCH_AT91RM9200
/* https://patchwork.kernel.org/patch/8113691/ */
#warning "AT91RM9200 has no F0 register (ATMEL_US_NAME). F0 in sama5d2"
#else
	...
	if (!strncmp(ip_name, usart, 4)) {
		...
		dev_err(port->dev, "Not supported ip name ->%s<-, set to uart\n", ip_name);
		return -EINVAL;
	}
#endif
